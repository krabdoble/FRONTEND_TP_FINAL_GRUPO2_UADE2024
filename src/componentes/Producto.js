...
function Producto(){
  ...
  const [fotoProducto, setFotoProducto] = useState(null); // Estado para la imagen
    ...
    useEffect(() => {
      axios.get('http://localhost:3000/api/proveedor/el-proveedor').then((res) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (editMode) {
       const updatedList = productoInfoList.map((item, index) => 
        ...
        try {
          await axios.post("http://localhost:3000/api/producto/guardar", formData, {
            headers: {
  ...
  const updateProducto = () => {
    axios.put(`http://localhost:3000/api/producto/modificar-producto/${id}`, {
      id: id,
      nombre: nombre,
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch(
        "http://localhost:3000/api/producto/el-producto"
      );
  ...
  const eliminarProductoInfo = (id) => {
    axios
      .delete(`http://localhost:3000/api/producto/eliminar/${id}`)
      .then(() => {
        setProductoInfoList(
